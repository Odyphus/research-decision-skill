#!/usr/bin/env python3
"""Frozen compatibility helpers for legacy Research Exploration state.
...
SCHEMA_VERSION = 2
SKILL_VERSION = "0.2.0-preview"
STATE_DIR_NAME = ".research-exploration"
...
LEGACY_STATE_FIELDS = tuple(
    ...
    if field not in {"domain", "search_runs", "expert_judgment_index"}
)
...
LEGACY_TRACE_HEADER = TRACE_HEADER.replace("Schema version: 2", "Schema version: 1")
LEGACY_EVIDENCE_HEADER = EVIDENCE_HEADER.replace("Schema version: 2", "Schema version: 1")
LEGACY_QUESTIONS_HEADER = QUESTIONS_HEADER.replace("Schema version: 2", "Schema version: 1")
...
def validate_domain(value: Any, context: str = "domain") -> dict[str, Any]:
    ...
    if name not in {"ai_ml", "stem_other", "other"}:
    ...
    expected = {
        "ai_ml": "public_source_decision_support_preview",
        "stem_other": "decision_framework_only",
        "other": "domain_judgment_unavailable",
    }[name]
    if status != expected:
...
def validate_search_run(record: Any, context: str = "search run") -> dict[str, Any]:
    ...
    if record["domain"]["name"] != "ai_ml" and record["source_evidence_ids"]:
        raise ValidationError(
            f"{context} cannot record domain judgments outside the AI/ML Preview"
        )
    return record
...
def validate_state(state: Any) -> dict[str, Any]:
    ...
    if state["skill_version"] != SKILL_VERSION:
        raise ValidationError(f"skill_version must be {SKILL_VERSION}")
...
def initial_state(
    ...
    *,
    ...
    domain_support_status: str = "domain_judgment_unavailable",
...
def load_state(path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise CorruptStateError(f"{state_path} is not valid UTF-8") from exc
    except OSError as exc:
        raise ReadOnlyStateError(f"cannot read {state_path}: {exc}") from exc
    value = strict_json_loads(text, context=str(state_path))
    return validate_state(value)


def load_state_read_only(path: str | Path) -> tuple[dict[str, Any], bool]:
    """Load v2 or a validated v1 state; the boolean marks legacy read-only data."""

    state_path = Path(path)
    text = _read_utf8(state_path)
    value = strict_json_loads(text, context=str(state_path))
    if isinstance(value, dict) and value.get("schema_version") == 1:
        return validate_legacy_state_v1(value), True
...
def workspace_lock(state_dir: Path, *, timeout: float = 5.0) -> Iterator[None]:
    ...
    while descriptor is None:
        try:
            ...
            if os.access(state_dir, os.W_OK) and time.monotonic() < deadline:
                time.sleep(0.01)
                continue
            if lock_path.exists():
                raise ConcurrentModificationError(
...
def _load_support_records(
    ...
    return ledger_records, question_records, search_runs, judgments
...
def init_workspace(
    ...
    *,
    ...
    workspace_paths = (*legacy_paths, expert_path)
    if all(path.is_file() for path in legacy_paths) and not expert_path.exists():
    ...
    present_before_lock = sum(path.exists() for path in workspace_paths)
    if present_before_lock not in {0, len(workspace_paths)}:
        present_names = sorted(path.name for path in workspace_paths if path.exists())
        missing_names = sorted(path.name for path in workspace_paths if not path.exists())
        raise ValidationError(
    ...
    with workspace_lock(state_dir, timeout=lock_timeout):
        present_under_lock = sum(path.exists() for path in workspace_paths)
...
def update_state(
    ...
    *,
    ...
    state_dir = Path(project_root).resolve() / STATE_DIR_NAME
    state_path = state_dir / STATE_FILE_NAME
    trace_path = state_dir / TRACE_FILE_NAME
    with workspace_lock(state_dir, timeout=lock_timeout):
        ...
        ledger_records, question_records, search_runs, judgments = (
            _load_support_records(state_dir)
        )
        _assert_support_indexes_consistent(
            current, ledger_records, question_records, search_runs, judgments
        )
        _assert_support_indexes_consistent(
            candidate, ledger_records, question_records, search_runs, judgments
        )
        ...
        if candidate["project"]["id"] != current["project"]["id"]:
            raise ValidationError("project.id is immutable in schema v2")
        if candidate["created_at"] != current["created_at"]:
            raise ValidationError("created_at is immutable in schema v2")
...
def append_decision(
    ...
    *,
    ...
    with workspace_lock(state_dir, timeout=lock_timeout):
        ...
        _assert_support_indexes_consistent(
            state, ledger_records, question_records, search_runs, judgments
        )
...
# Preferred v2 name; commit_route remains as a compatibility alias for callers.
...
def validate_workspace(project_root: str | Path) -> dict[str, Any]:
    ...
    missing = sorted(name for name, path in base_required.items() if not path.is_file())
    if missing:
    ...
    if is_legacy:
        ...
        if expert_path.exists():
            raise ValidationError(
                "mixed v1/v2 state workspace is not valid; no files were changed"
            )
        legacy = _validate_legacy_workspace_files(
            state,
            base_required[TRACE_FILE_NAME],
            base_required[EVIDENCE_FILE_NAME],
            base_required[QUESTIONS_FILE_NAME],
        )
        return {
            "status": "pass",
            ...
            "decision_record_count": len(legacy["trace_records"]),
            "evidence_count": len(legacy["evidence_records"]),
            "open_question_record_count": len(legacy["question_records"]),
            "migration": "not_available",
            "files": {
                name: sha256_file(path)
                for name, path in sorted(base_required.items())
            },
        }
    required = {
        **base_required,
    ...
    if not required[EXPERT_FILE_NAME].is_file():
        raise ValidationError(
            f"state workspace is missing files: ['{EXPERT_FILE_NAME}']"
        )
    evidence_ids = {item["evidence_id"] for item in state["evidence_index"]}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_parser.add_argument(
        "--domain",
        choices=["ai_ml", "stem_other", "other"],
        default="other",
    )
    init_parser.add_argument(
        "--support-status",
        choices=sorted(DOMAIN_SUPPORT_STATUSES),
        default="domain_judgment_unavailable",
    )
